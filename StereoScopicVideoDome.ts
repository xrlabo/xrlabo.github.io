import { VideoDome, Scene, Vector3, Vector2 } from "@babylonjs/core";
...
//特許をとる予定の技術
export default class StereoScopicVideoDome {
    IPD = 0.032;
    FadeRatio = 2.0;//一秒
    LeftEyeLeftVideoDome: VideoDome | null = null;
    LeftEyeRightVideoDome: VideoDome | null = null;
    RightEyeLeftVideoDome: VideoDome | null = null;
    RightEyeRightVideoDome: VideoDome | null = null;
    static deg2rad = (deg: number) => deg * (Math.PI / 180);
    static rad2deg = (rad: number) => rad * (180 / Math.PI);
    clamp = (num: number, min: number, max: number) => Math.min(Math.max(num, min), max);
    scene: Scene;
    constructor(scene: Scene) {
        this.scene = scene;
    }
    CreateVideoDome(leftVideo: HTMLVideoElement, rightVideo: HTMLVideoElement) {
        ...
        this.LeftEyeLeftVideoDome = new VideoDome(
        ...
        this.LeftEyeRightVideoDome = new VideoDome(
        ...
        this.RightEyeLeftVideoDome = new VideoDome(
        ...
        this.RightEyeRightVideoDome = new VideoDome(
        ...
        this.LeftEyeLeftVideoDome.mesh.rotation = new Vector3(0, 90, 0);
        this.LeftEyeRightVideoDome.mesh.rotation = new Vector3(0, 90, 0);
        this.RightEyeLeftVideoDome.mesh.rotation = new Vector3(0, 90, 0);
        this.RightEyeRightVideoDome.mesh.rotation = new Vector3(0, 90, 0);
    }
    render(headRad: number, cameraDegree: number) {
        headRad %= 2*Math.PI;
        ...
        //errorDeg = 0;
        this.LeftEyeLeftVideoDome.mesh.rotation = new Vector3(0, deltarad, 0);
        this.LeftEyeRightVideoDome.mesh.rotation = new Vector3(0, deltarad + errorRad/2, 0);
        this.RightEyeLeftVideoDome.mesh.rotation = new Vector3(0, deltarad, 0);
        this.RightEyeRightVideoDome.mesh.rotation = new Vector3(0, deltarad + errorRad/2, 0);
    }
    //これによって，カメラが映り込まない映像の生成をしています
    DomeRotate(cameraRad: number) {//cameraRadはTwincamの角度
        ...
        if (this.RightEyeLeftVideoDome == null) return;
        if (this.RightEyeRightVideoDome == null) return;
        this.LeftEyeLeftVideoDome.setAbsolutePosition(new Vector3(1000 - positionX, 0, -positionY));
        this.LeftEyeRightVideoDome.setAbsolutePosition(new Vector3(1000 + positionX, 0, positionY));
        this.RightEyeLeftVideoDome.setAbsolutePosition(new Vector3(1000 - positionX, 0, -positionY));
        this.RightEyeRightVideoDome.setAbsolutePosition(new Vector3(1000 + positionX, 0, positionY));
    }
    SetEyeRender(direction: LeftRight) {
        if (this.LeftEyeLeftVideoDome == null) return;
        if (this.LeftEyeRightVideoDome == null) return;
        if (this.RightEyeLeftVideoDome == null) return;
        if (this.RightEyeRightVideoDome == null) return;
    ...
    DomeFade(headRad: number, cameraDegree: number) {
    ...
    //Theta映像球切り替えロジック
    DomeFadeAlpha(nowAlpha: number, headDegree: number, cameraDegree: number, direction: LeftRight, isReverse: boolean) {
        let Alpha = nowAlpha;
    ...
    static CalcTwincamAngleError(ThetaM: number, PhiPe: number) {
        ThetaM = StereoScopicVideoDome.deg2rad(ThetaM);
        //ThetaM = 0;
        PhiPe = StereoScopicVideoDome.deg2rad(PhiPe);
        const IPD = 64;//64mm
        const focal_distance = 600;
        const Pe = new Vector2(focal_distance * Math.cos(PhiPe), focal_distance * Math.sin(PhiPe));
        const Or = new Vector2(Math.cos(ThetaM) * (IPD / 2), Math.sin(ThetaM) * (IPD / 2));
        const Ol = Or.multiplyByFloats(-1, -1);
        const Per = Pe.subtract(Or);
        const Pel = Pe.subtract(Ol);
        const PhiERR = Math.acos(Vector2.Dot(Per, Pel) / (Per.length() * Pel.length()));
        return PhiERR;
    }
    static ShieldingLimitDirection(ThetaM: number, PhiPe: number) {
        ThetaM = StereoScopicVideoDome.deg2rad(ThetaM);
        //ThetaM = 0;
        PhiPe = StereoScopicVideoDome.deg2rad(PhiPe);
        let pid = 64;
